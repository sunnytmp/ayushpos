...
App.factory('POService', ['$http', '$q',  '$location','$window', function($http, $q, $location, $window){
    ...
    return {
    	...
    	fetchAllPOS: function() {
    		        return $http.get(myaws +':8080/restpos/PurchaseOrder/')
                            .then(
                                    function(response){
                                        return response.data;
                                    }, 
            ...
            sendPoId: function( poid) {
             
            	if (typeof poid === 'undefined') {
            	 $window.location.href =  ":8080/viewpos/saleitems?val="+ 0	;
            	}else{
            		$window.location.href =  "/viewpos/Item?val="+ poid	;
            	}	
            },
            
            createPO: function(po){
                    return $http.post(myaws +':8080/restpos/PurchaseOrder/', po)
                            .then(
                                    function(response){
                                        return response.data;
                                    }, 
                                    function(errResponse){
                                        console.error('Error while creating PO');
                                        return $q.reject(errResponse);
                                    }
                            );
            },
            ...
            updatePO: function(po, id){
                    return $http.put(myaws+':8080/PurchaseOrder/'+id, po)
            ...
            deletePO: function(id){
                return $http.delete(myaws+':8080/restpos/PurchaseOrder/'+id)
                        .then(
                                function(response){
                                    return response.data;
                                }, 
